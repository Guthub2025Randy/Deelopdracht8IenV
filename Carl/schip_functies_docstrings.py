# -*- coding: utf-8 -*-
"""Schip_functies_docstrings.ipynb
...
def interpolerenLocatie(dictionary_ballasttank, vulling_tank, tanknummer):
    ...
    Inputs:
    dictionary_ballasttank: dictionary met data uit van de ballasttank (dictionary)
    vulling_tank: het volume van de vulling van de tank in m³ (float)
    tanknummer: het nummer van de tank (int)
    Returns:
...
def calculateWeightKraan(Krachten, Posities, H, kraan_lcg, SWLMax):
    ...
    Inputs:
    Krachten: lijst met de alle zwaartekrachten van de onderdelen van het schip (nog zonder de kraan)
    Posities: lijst met de alle zwaartepunten van de onderdelen van het schip (nog zonder de kraan)
    kraan_lcg: x-coördinaat van het zwaartepunt van de kraan (float)
    SWLMax: in N (float)
    Returns:
    Krachten: lijst met de alle zwaartekrachten van de onderdelen van het schip (nu met de kraan)
    Posities: lijst met de alle zwaartepunten van de onderdelen van het schip (nu met de kraan)
    """
    ZwaarteKheis = -SWLMax
...
def calculateMoment(positie,kracht):
    ...
    Inputs:
    postie: positievector (np.array)
    kracht: krachtvector (np.array)
    Returns:
    Moment: kruisproduct/moment (np.array)
    """
...
lente_schip: array met de lengtewaardes van het schip per centimeter (np.array)
Returns:
opwaartse_kracht_cm: array met de verdeelde opdrijvende kracht in N/m op elke centimeter van de lengte van het schip.
"""
...
def traagheidsmomentOverLengte(traagheidsmoment_csa_shell, Lengte_schip_csa_shell, lengte_schip):
  ...
  Inputs:
  traagheidsmoment_csa_shell: de traagheidsmomenten van de doorsnedes van de romp op bepaalde x-waarden (np.array)
  Lengte_schip_csa_shell: de x-waarden die overeenkomen met de waarden van de traagheidsmomenten (np.array)
  lengte_schip: array met de lengtewaardes van het schip per centimeter (np.array)
  Returns:
  traagheidsmoment_csa_shell_cm: het traagheidsmoment rond de y-as op elke centimeter lengte van het schip (np.array)
  """
...
def ballastLoop(x_in_m, opp, lengte_schip):
  ...
  Inputs:
  opp: oppervlakte van de dwarsdoorsnede van de het vullingswater in de tank op een aantal x-waarden (np.array)
  x_in_m: de overeenkomende x-waarden (np.array)
  lengte_schip: array met de lengtewaardes van het schip per centimeter (np.array)
  Returns:
...
def dwarskracht(q_x, lengte_schip):
    ...
    Inputs:
    q_x: netto verdeelde belasting op elke centimeter lengte van het schip (np.array)
    lengte_schip: array met de lengtewaardes van het schip per centimeter (np.array)
    Returns:
    dwarskracht: dwarskracht op elke cm van lengte van het schip (np.array)
    """
...
def buigendMoment(F_x, lengte_schip):
    ...
    Inputs:
    F_x: dwarskrachtlijn op elke centimeter lengte van het schip (np.array)
    lengte_schip: array met de lengtewaardes van het schip per centimeter (np.array)
    Returns:
    buigend_moment: buigend moment op elke cm van lengte van het schip (np.array)
    """
...
def reducMoment(M, I_traag):
    ...
    Inputs:
    M: buigend moment op elke centimeter lengte van het schip (np.array)
    I_traag: Traagheidsmoment van de doorsnede van het schip rond de y-as op elke centimeter lengte van het schip (np.array)
    Returns:
    gereduceerd_moment: (np.array)
    """
...
def hoekverdraaiingAcc(gereduceerd_moment_uitkomst, lengte_schip):
    ...
    Inputs:
    gereduceerd_moment_uitkomst: gereduceerd moment op elke centimeter lengte van het schip (np.array)
    lengte schip: array met de lengtewaardes van het schip per centimeter (np.array)
    Returns:
    phi_accent: phi' op elke centimeter lengte van het schip (np.array)
    """
...
def doorbuigingAcc(phi_accent, lengte_schip):
    ...
    lengte schip: array met de lengtewaardes van het schip per centimeter (np.array)
    Returns:
    w_acc: w' op elke centimeter lengte van het schip (np.array)
    """
...
def hoekverdraaiing(phi_acc, lengte_schip, C):
    ...
    Inputs:
    phi_acc: phi' op elke centimeter lengte van het schip (np.array)
    lengte_schip: array met de lengtewaardes van het schip per centimeter (np.array)
    C: ???
    Returns:
    phi: hoekverdraaiing phi op elke centimeter lengte van het schip (np.array)
    """
    phi = phi_acc + C
...
def doorbuiging(w_acc, lengte_schip, C):
    ...
    Inputs:
    w_acc: w' op elke centimter lengte van het schip (np.array)
    lengte_schip: array met de lengtewaardes van het schip per centimeter (np.array)
    C: ???
    Returns:
    w: doorbuiging w op elke centimeter lengte van het schip (np.array)
    """
...
def calculateSpiegel(arr_lengte, dic, huiddikte):
  ...
  Inputs:
  arr_lengte: array met de lengtewaardes van het schip per centimeter (np.array)
  dic: dictionary met de data uit het bestand hullareadata (dictionary)
  huiddikte: dikte van de huid in m (float)
  Returns:
  arr_gewicht: array met de verdeelde belasting in N/m van de spiegel op elke centimeter van de lengte van het schip (np.array)
  """
...
def calculateTrapezium(arr_lengte, dic_bh, huiddikte):
  ...
  Inputs:
  arr_lengte: array met de lengtewaardes van het schip per centimeter (array)
  dic_bh: de dictionary met de waarden uit het BHData bestand (dictionary)
  huiddikte: dikte van de huid in m (float)
  Returns:
  arr_gewicht: array met de verdeelde belasting van de tankschotten op elke centimeter lengte (array)
  """
  arr_gewicht = np.zeros(len(arr_lengte))
  for x in dic_bh:
    xmin = dic_bh[x][4]
...
def berekenKrachtVerdeling(lading_posities, massas, lengte_in_cm, straal):
    ...
    Inputs:
    lading_posities: de x-coördinaten van de zwaartepunten van de windmolentussenstukken (list)
    massas: de gewichten van de tussenstukken (list)
    lengte_in_cm: array met de x-coördinaat met van elke centimeter lengte van het schip (np.array)
    straal: straal van de windmolentussenstukken (float)"""
...
def calcParaboolFunctie(locatie, totaal_gewicht, arr_lengte, straal):
  ...
  Returns:
  q_out: array met de verdeelde belasting van het object op elke centimeter lengte van het schip (np.array)
  """
